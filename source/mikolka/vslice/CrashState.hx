...
class CrashState extends FlxState
{
	...
	{
		printToTrace('MELODY ${MainMenuState.melodyVersion}  (${error.message})');
		textNextY += 35;
		FlxTimer.wait(1 / 24, () ->
		{
			printSpaceToTrace();
			...
			printSpaceToTrace();
			printToTrace('REPORT TO GITHUB.COM/FNF-MELODYENGINE/MELODY-SOURCE');
	...
	static function saveError(error:CrashData)
	{
		...
		errMsg += '\n';
		errMsg += '\nPlease report this error to the GitHub page: https://github.com/fnf-melodyengine/melody-source\n\n> Crash Handler written by: sqirra-rng';

		#if !LEGACY_PSYCH
		@:privateAccess // lazy
        backend.CrashHandler.saveErrorMessage(errMsg + '\n');
		#else
		var path = './crash/' + 'Melody_' + dateNow + '.txt';
